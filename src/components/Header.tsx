...
export default function Header() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const sections = ['home', 'features', 'about', 'tokenomics', 'faq'];
  ...
  const socialLinks = [
    ...
    {
      name: 'DexScreener',
      href: 'https://dexscreener.com/base/0x9e8fc062c52444f9b7f194de283437fdf825073c',
      icon: (
    ...
    {
      name: 'DexTools',
      href: 'https://www.dextools.io/app/base/pair-explorer/0x9e8fc062c52444f9b7f194de283437fdf825073c',
      icon: (
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#0C0E21]/95 backdrop-blur-sm border-b border-white/5">
      <div className="container mx-auto px-6 py-4">
        <nav className="flex items-center justify-between">
          {/* Logo */}
          <button onClick={() => scrollToSection('home')} className="flex items-center space-x-3 cursor-pointer">
            ...
            <span className="text-white font-titillium text-2xl font-semibold">
            bid.launch
            </span>
          </button>
          ...
          <div className="hidden lg:flex items-center space-x-8">
            {navItems.map((item) => (
              <button 
                ...
                className={`relative transition-colors duration-300 font-lato cursor-pointer ${
              ...
              >
                {item.label}
                {activeSection === item.id && (
                  <div className="absolute -bottom-1 left-0 right-0 h-0.5 bg-gradient-to-r from-[#005EFF] to-[#303eb2] rounded-full"></div>
                )}
              </button>
            ))}
          </div>
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden bg-[#0C0E21]/98 backdrop-blur-sm border-t border-white/10">
            <div className="px-6 py-4 space-y-4">
              {navItems.map((item) => (
                <button 
                  ...
                  className={`block w-full text-left py-2 transition-colors duration-300 font-lato cursor-pointer ${
                    activeSection === item.id 
                      ? 'text-white' 
                      : 'text-white/80 hover:text-white'
                  }`}
                >
                  {item.label}
                  {activeSection === item.id && (
                    <div className="w-8 h-0.5 bg-gradient-to-r from-[#005EFF] to-[#303eb2] rounded-full mt-1"></div>
                  )}
                </button>
              ))}
